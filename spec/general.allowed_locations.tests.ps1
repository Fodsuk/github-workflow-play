...
Describe 'General Policies' -Tag "ci" {
    ...
    Context 'Policy: allowed' {
        ...
        It 'Resources cant be created in disallowed locations' {
           $outcome = ((az network vnet create --location EastUS --name $vnet_name --resource-group $resource_group_name --address-prefix 10.0.0.0/16 --subnet-name MySubnet --subnet-prefix 10.0.0.0/24 --output table)  2>&1 | Out-String)
           $outcome | Should -Match "'You can only create resources in UKSouth or UKWest'"
        }

        It 'Resources can be created in allowed locations' {
            $provisioning_state = az network vnet create --location UKSouth --name $vnet_name --resource-group $resource_group_name --address-prefix 10.0.0.0/16 --subnet-name MySubnet --subnet-prefix 10.0.0.0/24 --output tsv --query "newVNet.provisioningState"
