resource "random_string" "vnet_resource_group_name" {
  length  = 20
  special = false
}

resource "azurerm_resource_group" "vnet_resource_group" {
  name     = "policy-ci-${random_string.vnet_resource_group_name.result}"
  location = "UK South"
}
...
resource "azurerm_subnet" "pli_subnet" {
  name                 = "policy-ci-pli-${random_string.vnet_resource_group_name.result}"
  resource_group_name  = azurerm_resource_group.vnet_resource_group.name
  virtual_network_name = azurerm_virtual_network.vnet.name
  address_prefixes     = ["10.0.1.0/24"]
}

output "pli_subnet_id" {
